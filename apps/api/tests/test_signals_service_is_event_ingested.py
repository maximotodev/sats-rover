...
from app.services.signals_service import _is_event_ingested
...
class IsEventIngestedTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_v2_exists_and_present(self):
        db = _StubSession(v2_row={"one": 1})
        ingested = await _is_event_ingested(db=db, event_id="a" * 64)
        self.assertTrue(ingested)

    async def test_v2_exists_and_absent(self):
    ...
    async def test_v2_missing_falls_back_to_v1_present(self):
        ...
        db = _StubSession(v2_exc=exc, v1_row={"one": 1})
        ingested = await _is_event_ingested(db=db, event_id="a" * 64)
        self.assertTrue(ingested)

    async def test_v2_missing_falls_back_to_v1_absent(self):
        exc = self._dbapi_error(
            'relation "public.signals_v2_events" does not exist',
            pgcode="42P01",
        )
        db = _StubSession(v2_exc=exc, v1_row=None)
        ingested = await _is_event_ingested(db=db, event_id="a" * 64)
        self.assertFalse(ingested)
    ...
    async def test_non_eligible_dbapi_error_is_raised(self):
        ...
        with self.assertRaises(DBAPIError):
            await _is_event_ingested(db=db, event_id="a" * 64)
